...
const config = fs.existsSync(runControlFile)
  ? JSON.parse(fs.readFileSync(runControlFile))
  : defaultConfig;
...
const watcher = chokidar.watch(
  config.watchFolder || defaultConfig.watchFolder,
  {
...
const set = (obj, objectPath, val) => {
  ...
  p.forEach((key, index) => {
    ...
    if (
      isArray &&
      Object.prototype.toString.call(current[k]) !== "[object Array]"
    ) {
      current[k] = [];
    }
    if (index === length - 1) {
...
const createTranslations = () => {
  try {
    const translations = {};
    const localeFiles = walk(ROOT).sort((a, b) => a.length - b.length);
    const structures = getFilesStructures(localeFiles);
    Object.keys(structures).forEach((filePath) => {
      set(
        translations,
        structures[filePath].join("."),
        getTranslationFileContent(filePath)
      );
    });
    const targetFileContent = getTranslationFileContent(
      path.resolve(TARGET_FILE)
    )[[config.defaultLocale]];
    if (JSON.stringify(targetFileContent) === JSON.stringify(translations)) {
      console.log("no translations to update");
      return;
    }
    fs.writeFileSync(
      path.resolve(TARGET_FILE),
      generateTranslationFileContent({ [config.defaultLocale]: translations })
    );
...
const migrateTranslations = (fromLocale) => {
  try {
    ...
    const structures = getFilesStructures(localeFiles);
    const sourcesTranslations = getTranslationFileContent(
      path.resolve("src", "locales", `translation.${fromLocale}.yml`)
    )[fromLocale];
    Object.keys(structures).forEach((filePath) => {
      ...
      fs.writeFileSync(
        ...
        generateTranslationFileContent(content)
      );
    });
  } catch (e) {
    console.error(e);
...
const init = () => {
  ...
  console.log("watch on", config.watchFolder || defaultConfig.watchFolder);
  let watching = false;
